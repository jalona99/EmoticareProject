<%@ page contentType="text/html;charset=UTF-8" language="java" %>
    ...
    <body>
        ...
        <div class="container">
            ...
            <%-- Safe data bridge to avoid quote issues in JS string literals --%>
        ...
        <script>
            let questions = [];
            try {
                // Mengambil data dari script tag (Lebih aman dari quote escaping issues)
                const jsonText = document.getElementById('questionsData').textContent;
            ...
            function loadQuestion(index) {
                ...
                // Loop through scales (map as object)
                for (const val in scales) {
                    const label = scales[val];
                    const valueInt = parseInt(val);

                    // Cek apakah sudah pernah dijawab
                    const isChecked = (answers[q.id] !== undefined && answers[q.id] === valueInt);
                    const checkedAttr = isChecked ? "checked" : "";
                    const activeClass = isChecked ? "selected" : "";

                    html += '<label class="radio-option ' + activeClass + '" onclick="selectOption(this,' + q.id + ',' + valueInt + ')">';
                    html += '<input type="radio" name="answer" value="' + valueInt + '" ' + checkedAttr + '>';
                    html += '<span class="radio-label">' + label + '</span>';
                    html += '</label>';
                }
